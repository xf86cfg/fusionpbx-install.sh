...
if [ ."$database_host" = ."db" ] ; then
	...
	# add the databases, users and grant permissions to them
	psql --host=$database_host --port=$database_port --username=postgres -c "CREATE DATABASE fusionpbx;";
	psql --host=$database_host --port=$database_port --username=postgres -c "CREATE DATABASE freeswitch;";
	psql --host=$database_host --port=$database_port --username=postgres -c "CREATE ROLE fusionpbx WITH SUPERUSER LOGIN PASSWORD '$password';"
	psql --host=$database_host --port=$database_port --username=postgres -c "CREATE ROLE freeswitch WITH SUPERUSER LOGIN PASSWORD '$password';"
	psql --host=$database_host --port=$database_port --username=postgres -c "GRANT ALL PRIVILEGES ON DATABASE fusionpbx to fusionpbx;"
	psql --host=$database_host --port=$database_port --username=postgres -c "GRANT ALL PRIVILEGES ON DATABASE freeswitch to fusionpbx;"
	psql --host=$database_host --port=$database_port --username=postgres -c "GRANT ALL PRIVILEGES ON DATABASE freeswitch to freeswitch;"
	# ALTER USER fusionpbx WITH PASSWORD 'newpassword';
fi
